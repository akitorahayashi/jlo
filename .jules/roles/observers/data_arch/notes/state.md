...
*   **Issues**:
    *   **Infrastructure Leakage**:
        *   `src/domain/setup.rs` entities (`EnvSpec`, `SetupConfig`, `ComponentMeta`) derive `serde::Deserialize` and use `#[serde(default)]`.
        *   `src/domain/run_config.rs`: `RunConfig` depends on `toml` crate via `parse_toml` method.
...
| **Boundary Sovereignty** | ❌ Failed | Domain models depend on `serde` and `toml`; Services mix adapters and logic. |
| **Temporal Monotonicity** | ⚠️ At Risk | Mutable state in `ArboardClipboard` adapter. |

## Verification Log

*   **2026-01-30**: Verified all findings. Confirmed `ComponentMeta` exists in `src/domain/setup.rs` and leaks `serde` details. Confirmed `src/services` mixes domain services and adapters. Confirmed `DiscoveredRole` uses `String`. Confirmed `RunConfig` depends on `toml` crate.
