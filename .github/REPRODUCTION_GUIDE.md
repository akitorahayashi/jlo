...
Copying these files is recommended to suppress automated code reviews on Jules-managed PRs (e.g., Observer/Decider/Planner) while keeping them active for human Implementer PRs.

| File | Purpose |
|------|---------|
| `.gemini/config.yml` | Filter automated reviews based on branch patterns |
| `.coderabbit.yaml` | Configure on-demand reviews for Jules branches |
