...
#[test]
fn role_add_registers_role_and_updates_schedule() {
    let ctx = TestContext::new();

    ctx.init_remote();

    ctx.cli()
        .args(["role", "add", "observers", "pythonista"])
        .assert()
        .success()
        .stdout(predicate::str::contains(".jlo/config.toml"));

    let role_path = ctx.jlo_path().join("roles/observers/pythonista/role.yml");
    assert!(role_path.exists(), "Built-in role should be materialized under .jlo/roles by add");
