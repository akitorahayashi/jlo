...
/// Options for `workflow process pr enable-automerge`.
#[derive(Debug, Clone)]
...
/// Output of `workflow process pr enable-automerge`.
...
pub fn execute(
    ...
    if branch_policy_for_head(&pr.head).is_none() {
        return Ok(skip(
            options.pr_number,
...
fn branch_policy_for_head(head: &str) -> Option<&'static str> {
    BRANCH_PREFIXES.iter().copied().find(|prefix| head.starts_with(prefix))
}
