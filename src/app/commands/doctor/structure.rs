...
fn check_version_file(jules_path: &Path, current_version: &str, diagnostics: &mut Diagnostics) {
    ...
    let runtime_version = Version::parse(&content);
    let current_version_obj = Version::parse(current_version);
    ...
    if current_version_obj.is_none() {
        diagnostics
            .push_error(version_path.display().to_string(), "Current binary version is invalid");
        return;
    }

    if runtime_version.unwrap() > current_version_obj.unwrap() {
        diagnostics.push_error(
