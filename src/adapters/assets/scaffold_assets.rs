...
///
/// Also maps `.jules/roles/` → `jules/layers/` since the source uses `layers/`
/// while the deployment target uses `roles/`.
fn unmap_scaffold_path(path: &str) -> String {
    if let Some(rest) = path.strip_prefix(".jlo/") {
        format!("jlo/{}", rest)
    } else if let Some(rest) = path.strip_prefix(".jules/roles/") {
        format!("jules/layers/{}", rest)
