...
mod tests {
    ...
    impl GitHubPort for FakeGitHub {
        ...
        fn get_pr_detail(&self, _pr_number: u64) -> Result<crate::ports::PullRequestDetail, AppError> {
             Ok(crate::ports::PullRequestDetail { number: 101, head: String::new(), base: String::new(), is_draft: false, auto_merge_enabled: false })
        }
    ...
    fn mock_decider_processes_events_and_creates_requirements() {
        ...
        workspace.write_file(".jules/exchange/events/pending/mock-mock-test-decider-event1.yml", "id: event1\nsummary: s1").unwrap();
        workspace.write_file(".jules/exchange/events/pending/mock-mock-test-decider-event2.yml", "id: event2\nsummary: s2").unwrap();
        ...
        let planner_req = req_files.iter().find(|p| p.to_string_lossy().contains("mock-planner-mock-test-decider")).expect("planner req missing");
        let impl_req = req_files.iter().find(|p| p.to_string_lossy().contains("mock-impl-mock-test-decider")).expect("implementer req missing");
        ...
        assert!(!workspace.file_exists(".jules/exchange/events/pending/mock-mock-test-decider-event1.yml"));
        assert!(workspace.file_exists(".jules/exchange/events/decided/mock-mock-test-decider-event1.yml"));
    }
    ...
    fn mock_decider_fails_with_insufficient_events() {
        ...
        // Only 1 event
        workspace.write_file(".jules/exchange/events/pending/mock-mock-test-decider-event1.yml", "id: event1").unwrap();
        ...
        assert!(matches!(result, Err(AppError::Validation(msg)) if msg.contains("requires at least 2 decided events")));
    }
}
